...
void raw_trimming(std::string config_file) {
    ...
    TCut cutexpression = "bb.ps.e>0.2&&fabs(bb.etot_over_p[0]-1.0)<0.3&&sbs.hcal.e>0.02&&fabs(bb.tr.vz[0])<0.27&&bb.ps.nblk>0&&bb.sh.nblk>0&&sbs.hcal.nblk>0";
    // TCut cutexpression = "";
    ...
    std::string input_dir = settings["input_dir"];
    std::string output_dir = settings["output_dir"];
    ...
    TFile *outputFile = new TFile(output_filepath.c_str(), "RECREATE");
    ...
    outputTree->Branch("sbs.hcal.nblk", &sbs_hcal_nblk, "sbs.hcal.nblk/D");
    ...
    outputTree->Branch("bb.ps.nblk", &bb_ps_nblk, "bb.ps.nblk/D");
    outputTree->Branch("bb.sh.nblk", &bb_sh_nblk, "bb.sh.nblk/D");
    ...
    TTreeFormula *GlobalCut = new TTreeFormula("GlobalCut", cutexpression, C);
    C->SetBranchStatus("*",0);
    ...
    C->SetBranchAddress("sbs.hcal.nblk", &sbs_hcal_nblk_in);
    ...
    double bb_ps_e_in;
    double bb_tr_n_in;
    double bb_sh_nblk_in;
    double bb_ps_nblk_in;
    ...
    C->SetBranchAddress("bb.ps.nblk", &bb_ps_nblk_in);
    C->SetBranchAddress("bb.sh.nblk", &bb_sh_nblk_in);
    ...
    if(eventTracker ==0 || treenum != previousTreeNum){
        std::cout << "Now processing tree number: " << treenum << std::endl;
        previousTreeNum = treenum;
        GlobalCut->UpdateFormulaLeaves();
    }
    bool passedGlobalCut = GlobalCut->EvalInstance(0) !=0;
	...
	dy_clus[l] = sbs_hcal_clus_y_in[l] - sbs_hcal_y_exp;
      ...
      TLorentzVector Pprime_pseudo(Pprime_pseudo_vec, Pprime_pseudo_E);
    ...
    ++eventTracker;
    if (eventTracker % (totalEvents / 100) == 0) {
    ...
    std::cout << "Merged ROOT file created: " << output_filepath << std::endl;
}
