...
void raw_single_trimming(std::string config_file) {
    ...
    if (config == "kin2") {
        selected_numbers = {2200};
    }
    else if (config == "kin3") {
         selected_numbers = {2692};
    }
    else if (config == "kin4a") {
         selected_numbers = {3833};
    }
    else if (config == "kin4b") {
         selected_numbers = {5514};
    }
   ...
   // Directory containing the files
    ...
    std::string output_filename = output_dir + "QE_data_" + exp_name + "_run_" + std::to_string(selected_numbers[0]) +  "_sbs100p_" + target + ".root";
