...
void raw_test_trimming(std::string config_file) {
    ...
    std::vector<std::string> branches_to_extract_scalar = {"bb.grinch_tdc.clus.trackindex", "bb.grinch_tdc.clus.size", "bb.sh.e", "bb.sh.atimeblk", "bb.ps.e", "sbs.hcal.e", "sbs.hcal.x", "sbs.hcal.y", "sbs.hcal.rowblk", "sbs.hcal.colblk", "sbs.hcal.idblk", "sbs.hcal.tdctimeblk", "sbs.hcal.atimeblk", "bb.tr.n", "scalhel.hel", "sbs.hcal.nclus", "sbs.tdctrig.rftime", "bb.tdctrig.rftime","e.kine.W2","e.kine.Q2","g.trigbits"}; // Modify as needed

    std::vector<std::string> branches_to_extract_array = {"bb.tr.p", "bb.tr.px", "bb.tr.py", "bb.tr.pz", "bb.tr.vx", "bb.tr.vy", "bb.tr.vz", "bb.tdctrig.tdcelemID", "bb.tdctrig.tdc",  "bb.hodotdc.clus.bar.tdc.meantime", "bb.gem.track.nhits", "bb.etot_over_p", "sbs.hcal.clus_blk.tdctime_tw","sbs.hcal.clus_blk.tdctime"};
  
   // Directory containing the files
    std::string input_dir = "/lustre24/expphy/volatile/halla/sbs/sbs-gen/GEN_REPLAYS/" + pass + "/TEST/try7/" + exp_name + "/rootfiles/";  // cache directory for Raw data
    ...
    std::map<std::string, std::vector<Double_t>> outArrays;
        ...
        if (!file->IsDirectory() && fname.EndsWith(".root")) {
		    ...
		    for (const auto& branch : branches_to_extract_array) {
		        arrayReaders[branch] = std::make_unique<TTreeReaderArray<Double_t>>(reader, branch.c_str());
                    ...
                    while (reader.Next()) {
		        auto& vz_array = *arrayReaders["bb.tr.vz"];
			auto& hcale = **scalarReaders["sbs.hcal.e"];

			if (vz_array.GetSize() == 0 || std::abs(vz_array[0]) >= 0.27) {
			    continue;
			}
			...
			if (hcale <= 0.025) {
			    ...
			    outArrays[name].assign(readerPtr->begin(), readerPtr->end());
                        }
			outputTree->Fill();
                    }
