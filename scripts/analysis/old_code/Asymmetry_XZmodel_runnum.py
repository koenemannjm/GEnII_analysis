...
for kine in kine_list:
    ...
    with PdfPages(f"fig_asymmetry_runnum_{kine}.pdf") as pdf:
        for j in range(2):
            ...
            bins_dy = np.histogram_bin_edges(dy_cut, bins='auto')
            bins_W2 = np.histogram_bin_edges(W2_cut, bins='auto')
            ...
            for run_number in unique_runnum:
                single_runnum = (runnum_cut == run_number)

                true_helicity_cut_run = true_helicity_cut[single_runnum]
                W2_cut_run = W2_cut[single_runnum]
                dx_cut_run = dx_cut[single_runnum]
                
                up_hel_cut = (true_helicity_cut_run == 1)
                W2_up = W2_cut_run[up_hel_cut]
                dx_up = dx_cut_run[up_hel_cut]
                dwn_hel_cut = (true_helicity_cut_run == -1)
                W2_dwn = W2_cut_run[dwn_hel_cut]
                dx_dwn = dx_cut_run[dwn_hel_cut]

                Nevents_avg = len(W2_cut_run)
                bins_W2_run = np.histogram_bin_edges(W2_cut_run, bins='auto')
                bins_dx_run = np.histogram_bin_edges(dx_cut_run, bins='auto')
                # nbins = int(np.sqrt(Nevents_avg))
                Counts_up, xedges_up = np.histogram(W2_up, bins=bins_W2_run)
                Counts_dwn, xedges_dwn = np.histogram(W2_dwn, bins=bins_W2_run)
                Counts_dx_up, dxedges_up = np.histogram(dx_up, bins=bins_dx_run)
                Counts_dx_dwn, dxedges_dwn = np.histogram(dx_dwn, bins=bins_dx_run)

                W2_centers_up_val = 0.5*(xedges_up[1:] + xedges_up[:-1])
                W2_centers_dwn_val = 0.5*(xedges_dwn[1:] + xedges_dwn[:-1])
                W2_centers_val = 0.5*(W2_centers_up_val + W2_centers_dwn_val)

                dx_centers_up_val = 0.5*(dxedges_up[1:] + dxedges_up[:-1])
                dx_centers_dwn_val = 0.5*(dxedges_dwn[1:] + dxedges_dwn[:-1])
                dx_centers_val = 0.5*(dx_centers_up_val + dx_centers_dwn_val)
                
                runnum_filtered_val = np.repeat(run_number, len(W2_centers_val))
                runnum_filtered_dx_val = np.repeat(run_number, len(dx_centers_val))

                W2_centers_list.append(W2_centers_val)
                dx_centers_list.append(dx_centers_val)
                Counts_up_list.append(Counts_up)
                Counts_dwn_list.append(Counts_dwn)
                Counts_dx_up_list.append(Counts_dx_up)
                Counts_dx_dwn_list.append(Counts_dx_dwn)
                runnum_list.append(runnum_filtered_val)
                runnum_dx_list.append(runnum_filtered_dx_val)
                
                

            Counts_up_list = [item for sublist in Counts_up_list for item in sublist]
            Counts_dwn_list = [item for sublist in Counts_dwn_list for item in sublist]
            Counts_dx_up_list = [item for sublist in Counts_dx_up_list for item in sublist]
            Counts_dx_dwn_list = [item for sublist in Counts_dx_dwn_list for item in sublist]
            runnum_list = [item for sublist in runnum_list for item in sublist]
            runnum_dx_list = [item for sublist in runnum_dx_list for item in sublist]
            W2_centers_list = [item for sublist in W2_centers_list for item in sublist]
            dx_centers_list = [item for sublist in dx_centers_list for item in sublist]

            Counts_up = np.array(Counts_up_list)
            errCounts_up = np.sqrt(Counts_up)
            Counts_dwn = np.array(Counts_dwn_list)
            errCounts_dwn = np.sqrt(Counts_dwn)
            Counts_dx_up = np.array(Counts_dx_up_list)
            errCounts_dx_up = np.sqrt(Counts_dx_up)
            Counts_dx_dwn = np.array(Counts_dx_dwn_list)
            errCounts_dx_dwn = np.sqrt(Counts_dx_dwn)
            runnum_filtered = np.array(runnum_list)
            runnum_dx_filtered = np.array(runnum_dx_list)
            W2_centers = np.array(W2_centers_list)
            dx_centers = np.array(dx_centers_list)
            ...
            del runnum_list
            del W2_centers_val
            del runnum_filtered_val
            del dx_centers_list
            del Counts_dx_up_list
            del runnum_dx_list
            del Counts_dx_dwn_list
            del W2_centers_up_val
            del W2_centers_dwn_val
            ...
            runnum_filtered = runnum_filtered[noZeros]
            ...
            smallerror = (np.abs(relerrAsymmetry) < accep_max_error)
            
            Asymmetry = Asymmetry[smallerror]
            W2_centers = W2_centers[smallerror]
            errAsymmetry = errAsymmetry[smallerror]
            runnum_filtered = runnum_filtered[smallerror]

            print(f"Making plot W2 {label1[j]}...")
            ...
            ax = ax.flatten()
            chunk_size = len(runnum_filtered) // 4
            for chunk in range(4):
                start = chunk * chunk_size
                end = (chunk + 1) * chunk_size if chunk < (4 - 1) else len(runnum_filtered)
                ax[chunk].errorbar(runnum_filtered[start:end],Asymmetry[start:end],
                        yerr=errAsymmetry[start:end],
                        fmt='o', capsize=3, markersize=4,label=f"data")
                ax[chunk].set_xlabel("runnum")
                ax[chunk].set_ylabel("Asymmetry")
                ax[chunk].set_title(f"Raw W2 Asymmetry vs run numbers {int(runnum_filtered[start])} - {int(runnum_filtered[end-1])} kin = {expname} ({label1[j]})")
                ax[chunk].minorticks_on()
                ax[chunk].grid(True)
                ax[chunk].legend()
            pdf.savefig(fig, bbox_inches='tight')
            plt.close(fig)
            ...
            # ----- dx PLOTS -----
            noZeros = (Counts_dx_up != 0) & (Counts_dx_dwn != 0)
            ...
            runnum_filtered = runnum_dx_filtered[noZeros]
            ...
            print(f"Making plot dx {label1[j]}...")
            fig, ax = plt.subplots(figsize=(14, 8))
            
            ax.errorbar(dx_centers, Asymmetry, yerr=errAsymmetry,
                         fmt='ko', ecolor='k', capsize=3, markersize=4, label="data")
            ax.set_xlabel("dx")
            ...
            for chunk in range(4):
                ...
                ax[chunk].set_xlabel("runnum")
                ax[chunk].set_ylabel("Asymmetry")
                ax[chunk].set_title(f"Raw dx Asymmetry vs run numbers {int(runnum_filtered[start])} - {int(runnum_filtered[end-1])} kin = {expname} ({label1[j]})")
                ax[chunk].minorticks_on()
                ax[chunk].grid(True)
                ax[chunk].legend()
            pdf.savefig(fig, bbox_inches='tight')
