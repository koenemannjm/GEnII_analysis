...
void compute_dx_dy(std::string config, std::string target, std::string pass){
    ...
    for (Long64_t i = 0; i < Nentries; ++i) {
      tree->GetEntry(i);
      if (abs(tr_vz) <= 0.27 && Hcale > 0.025 && eprime_pse > 0.2) {
	  ...
	  if (e_over_p >= 0.85 && e_over_p <= 1.15) {
	    h_W2_cut->Fill(W2);
	    h_Q2_cut->Fill(Q2);
